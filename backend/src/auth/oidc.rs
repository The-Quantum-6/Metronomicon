    use crate::{repositories::user::{create_user, get_user_by_sub}, state::AppState};
    use axum::{
        extract::{Query, State},
        response::Redirect,
    };
...
// Clean up temporary authentication artifacts from the session
    ...
    session
        .insert("user_sub", user_sub.clone())
        .await
        .map_err(|e| {
            (
                axum::http::StatusCode::INTERNAL_SERVER_ERROR,
