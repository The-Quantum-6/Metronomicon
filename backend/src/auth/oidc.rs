...
pub async fn login_send(session: Session, State(client): State<AppState>) -> Result<Redirect, (axum::http::StatusCode, String)> {
...
pub async fn login_callback(Query(params): Query<CallbackParams>, State(app_state): State<AppState>, session: Session) -> Result<Redirect, (axum::http::StatusCode, String)> {
