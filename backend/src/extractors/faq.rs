...
use crate::aggregates::faq::command::FaqCommand;

pub struct FaqCommandExtractor(pub HashMap<String, String>, pub FaqCommand);

// const USER_AGENT_HDR: &str = "User-Agent";

impl<S> FromRequest<S> for FaqCommandExtractor
where
    S: Send + Sync,
{
    type Rejection = FaqCommandExtractionError;
    ...
    async fn from_request(req: Request<Body>, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let command: FaqCommand = serde_json::from_value(json_value)?;
        Ok(Self(metadata, command))
    }
}

pub struct FaqCommandExtractionError;

impl IntoResponse for FaqCommandExtractionError {
...
impl From<axum::extract::rejection::BytesRejection> for FaqCommandExtractionError {
    fn from(_: axum::extract::rejection::BytesRejection) -> Self {
        Self
    }
}

impl From<serde_json::Error> for FaqCommandExtractionError {
